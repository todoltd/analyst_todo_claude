...
- **Часті стандартні моделі (не вигадуй / не плутай назви):** контакт — `res.partner`; продаж —
  `sale.order`/`sale.order.line`; закупівля — `purchase.order`; склад — `stock.picking`/`stock.move`/`stock.move.line`;
  **наявність/залишки — `stock.quant`** (а `product.product.qty_available`/`virtual_available` — обчислювані
  з нього, а НЕ окрема таблиця залишків); виробництво — `mrp.production`/`mrp.bom`; CRM — `crm.lead`;
  рахунок — `account.move`/`account.move.line`. Звіряй точне написання (часті одруки корпусу: `parther_id`,
  `tock.quant`, `srm.lead`, `mrp.boom`).
