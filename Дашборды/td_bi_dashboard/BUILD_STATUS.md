...
**Реально лишилось (свідомо, НЕ gaps):** (1) `right/full/cross` join — **за AC-38 НЕДОСТУПНІ**; guard, що їх відхиляє, — це і є коректна реалізація (додати = порушити ТЗ). (2) DSL-міри у бленді — єдине справжнє Stage-3-розширення: потребує CTE-кваліфікації колонок у `_compile_formula` + семантики ре-агрегації; **корректність-критичне, нішеве**; наразі безпечно відхиляється UserError. Серверне обслуговування з предагрегата для m2o/date-вимірів і RLS-кейсу — також Stage-3 (потребує відтворення форматування formatted_read_group + RLS-домену на предагрегаті); safety-частини зроблені, scalar-no-rules-кейс обслуговується.

**ПІДСУМОК МОДУЛЯ:** реалізовано Stage-1 + Stage-2 повністю — аналітичний двигун (агрегації/DSL/time-intelligence/show_as/бленди/мультивалютність), інтерактивний фронтенд, розсилки (алерти+підписки), експорт (XLSX+PDF), публічний доступ (заморожені посилання+HTTP), матеріалізація (детектор+RLS-guard+serve безпечного кейсу), нативні GROUPING SETS. **71 AC-тест, усе на живій Odoo 19, 9 комітів на `feat/bi-dashboards`.** Поза межами лишилося тільки спец-заборонене (#1) і одне нішеве Stage-3-розширення (DSL-у-бленді).
