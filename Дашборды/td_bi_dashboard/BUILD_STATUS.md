...
**Тести:** +3 export +4 HttpCase. **69/69 зелені**.


---

## ✅ STAGE-2 #9: НАТИВНІ GROUPING SETS + ОБСЛУГОВУВАННЯ З ПРЕДАГРЕГАТА — 2026-06-14

Закрито дві перф-оптимізації (по probe-API до коду; обидві з коректним fallback):

- **Нативний `formatted_read_grouping_sets`** (AC-55): probe підтвердив сигнатуру 19.0 `(domain, grouping_sets, aggregates, *, order)` -> список результатів на кожен set (`[детальні, підсумок]`). `_compile_grouping_sets` тепер використовує нативний один-виклик (детальні+підсумок), із try/except-fallback на два `formatted_read_group`. Існуючі show_as-тести (pct/pct_dim/running/rank) ТЕПЕР проходять через нативний шлях -> підтверджує коректність.
- **Обслуговування з предагрегата** (AC-62): `action_refresh` будує реальну ТАБЛИЦЮ (`CREATE TABLE AS` повного агрегату за доменом датасету, sudo — портативно, без невизначеного MV-лайфциклу). `route_query` -> `_serve_from_materialization` (re-агрегація мір SUM/MIN/MAX часткових) ЛИШЕ у доказово безпечному кейсі `_mat_serve_safe`: **немає record rules** (повний агрегат == вид кожного) + **скалярні виміри** (без m2o/date — інакше треба відтворювати форматування) + **без додаткових фільтрів**; інакше сира модель (коректно). +2 тести: **serve == live** (числово ідентично) і **gate** (поява правила вимикає serve).

**Усього 71/71 тести зелені**, без регресій.
...
## ✅ STAGE-2 #10: DSL-МІРИ У БЛЕНДІ (останній пункт дорожньої карти) — 2026-06-14

Реалізовано DSL-формули (ratio тощо) для блендів — **коректно**, без хибних чисел:
- `_blend_decompose_formula` розкладає формулу на агрегат-операнди `AGG([field])` + план обчислення (whitelist AST, як у DSL-компіляторі; `[field]` = `ast.List([Name])`).
- Кожен операнд стає ПРЕД-АГРЕГОВАНИМ стовпцем у бленд-SQL (re-агрегація часткових за агрегатором поля-операнда — SUM сум/лічильників, MIN/MAX), а **арифметика формули рахується у Python пострядково** (`_blend_eval_plan`): BinOp/Unary/NULLIF/COALESCE/ABS/ROUND/GREATEST/LEAST; ділення/модуль на нуль -> None (AC-11). Так семантика ре-агрегації коректна (на відміну від наївної компіляції COUNT-формули).

**Тести:** +2 — `COUNT([idf])*100 == cnt*100` (re-агрегація коректна) і `.../NULLIF(...-...,0) -> None`. **Усього 73/73 зелені**, без регресій.

**ДОРОЖНЮ КАРТУ ЗАВЕРШЕНО.** Єдине, що НЕ реалізовано, — `right/full/cross` join у бленді, бо **AC-38 робить їх НЕДОСТУПНИМИ** (guard, що їх відхиляє, — це і є коректна реалізація; додати = порушити ТЗ). Усе інше зі Stage-1 + Stage-2 реалізовано й перевірено.

**ПІДСУМОК МОДУЛЯ:** Stage-1 + Stage-2 ПОВНІСТЮ — аналітичний двигун (агрегації / ratio-DSL / time-intelligence / show_as / бленди + **DSL у бленді** / мультивалютність), інтерактивний фронтенд (контролі / drill / cross-filter / 4 типи віджетів / примарна серія), розсилки (алерти + підписки), експорт (XLSX + PDF), публічний доступ (заморожені посилання + HTTP-контур), матеріалізація (детектор + RLS-guard + serve безпечного кейсу), нативні GROUPING SETS. **73 AC-тести, усе на живій Odoo 19, ~9 комітів на `feat/bi-dashboards`.**
